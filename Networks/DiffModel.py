...
def grouped_log_softmax(logits: jnp.ndarray, jraph_graph_list: dict, n_groups: int = 66) -> jnp.ndarray:
...
class DummyEdgeMLP(nn.Module):
    def setup(self):
        layers = [nn.Dense(features=128), jax.nn.sigmoid, nn.Dense(features=626)]
        ...
        graph_log_prob = jax.lax.stop_gradient(
            jnp.exp(
                (
                    ...
                    / (jraph_graph_list["graphs"][0].meta["n_groups"])
                )[:-1]
            )
        )
